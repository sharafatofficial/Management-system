  ...
  <div class="container p-4 bg-white" style="max-width:2000px">
    <div class="row">
        ...
        <div class="col-12 pt-4">
            <form action="{{url('admin/team/update/'.$team->id)}}" method="post" class="w-100">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="exampleInputEmail1">Team Name</label>
                            <input type="text" name="name" value="{{$team->name}}"  class="form-control" placeholder="First Name">	
                            @if ($errors->has('name'))
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="supervisor" aria-label="Default select example">
                                ...
                                <option value="{{$super->id}}" {{($team->supervisor==$super->id) ? 'Selected' :''}}>{{$super->name}}</option> 
                                @endforeach 
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="employee[]" multiple>
                                @foreach($employe as $emp)
                                    <option value="{{$emp->id}}"{{is_array($team->employee) && in_array($emp->id, $team->employee) ? 'selected' : '' }}>{{$emp->name}}</option>
                                @endforeach
                            </select>
